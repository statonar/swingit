...
Dependencies:
    pip install streamlit finviz yfinance pandas pandas-ta requests plotly
"""
...
from plotly.subplots import maåke_subplots
import requests
...
<style>
    ...
    [data-testid="stMetricLabel"] { color: var(--muted) !important; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.06em; }
...
with st.sidebar:
    ...
    ma_signal = st.selectbox(
        "MA crossover signal",
        ["Any", "Bullish (50 > 200)", "Bearish (50 < 200)", "Price above 50-day", "Price above 200-day"],
    )
    ...
    st.markdown(f"<span class='pill'>Finviz → yfinance pipeline</span>", unsafe_allow_html=True)
...
def fetch_finviz_tickers(exchanges: list[str], price_min: int, price_max: int, vol_min: int, limit: int) -> list[str]:
    """Pull tickers from Finviz screener matching basic price/volume filters."""
    try:
        filters = [
            f"exch_{EXCHANGE_MAP.get(ex, 'nyse')}" for ex in exchanges
        ] + [
            f"fa_curratio_o1",          # liquid stocks
        ]

        # Use finviz Screen with price & volume filters via custom URL params
        screen = Screen.init_from_filters(
            filters=filters,
            order="price",
        )
...
def compute_technicals(ticker: str) -> dict | None:
    """Download 1y of daily OHLCV and compute RSI, MACD, MAs."""
    try:
        ...
        if macd_obj is not None:
            cols = macd_obj.columns.tolist()
            macd_line   = float(macd_obj[cols[0]].iloc[-1])
            signal_line = float(macd_obj[cols[1]].iloc[-1])
            # previous bar cross detection
            macd_prev   = float(macd_obj[cols[0]].iloc[-2])
            sig_prev    = float(macd_obj[cols[1]].iloc[-2])
            macd_cross  = (
                "bullish" if macd_prev < sig_prev and macd_line > signal_line else
                "bearish" if macd_prev > sig_prev and macd_line < signal_line else
                "none"
            )
        else:
            macd_cross = "none"
...
def passes_filters(row: dict) -> bool:
    """Apply user-selected technical filters."""
    rsi = row.get("rsi")
    ...
    if ma_signal == "Bullish (50 > 200)":
        if not (sma50 and sma200 and sma50 > sma200):
            return False
    elif ma_signal == "Bearish (50 < 200)":
        if not (sma50 and sma200 and sma50 < sma200):
            return False
    elif ma_signal == "Price above 50-day":
        if not (sma50 and price > sma50):
            return False
    elif ma_signal == "Price above 200-day":
        if not (sma200 and price > sma200):
            return False

    cross = row.get("macd_cross", "none")
    if macd_signal == "Bullish cross" and cross != "bullish":
        return False
    if macd_signal == "Bearish cross" and cross != "bearish":
        return False

    return True


def signal_pills(row: dict) -> str:
    pills = []
    rsi = row.get("rsi")
    if rsi:
        if rsi < 30:
            pills.append("<span class='pill pill-green'>Oversold RSI</span>")
        elif rsi > 70:
            pills.append("<span class='pill pill-red'>Overbought RSI</span>")

    sma50, sma200 = row.get("sma50"), row.get("sma200")
    if sma50 and sma200:
        if sma50 > sma200:
            pills.append("<span class='pill pill-green'>Golden Cross</span>")
        else:
            pills.append("<span class='pill pill-red'>Death Cross</span>")

    cross = row.get("macd_cross", "none")
    if cross == "bullish":
        pills.append("<span class='pill pill-green'>MACD ↑</span>")
    elif cross == "bearish":
        pills.append("<span class='pill pill-red'>MACD ↓</span>")

    return "".join(pills) if pills else "<span class='pill'>—</span>"


def mini_chart(data: dict) -> go.Figure:
    df = data["df"].copy()
    close = df["Close"].squeeze()
    dates = df.index
    ...
    if sma50_s is not None:
        fig.add_trace(go.Scatter(x=dates, y=sma50_s,  line=dict(color="#58a6ff", width=1.5), name="SMA 50"),  row=1, col=1)
    if sma200_s is not None:
        fig.add_trace(go.Scatter(x=dates, y=sma200_s, line=dict(color="#e3b341", width=1.5), name="SMA 200"), row=1, col=1)

    # RSI
    if rsi_s is not None:
        fig.add_trace(go.Scatter(x=dates, y=rsi_s, line=dict(color="#a371f7", width=1.5), name="RSI"), row=2, col=1)
    ...
    if macd_obj is not None:
        cols = macd_obj.columns.tolist()
        macd_l = macd_obj[cols[0]]
        sig_l  = macd_obj[cols[1]]
        hist   = macd_obj[cols[2]] if len(cols) > 2 else macd_l - sig_l
        ...
        fig.add_trace(go.Scatter(x=dates, y=macd_l, line=dict(color="#58a6ff", width=1.5), name="MACD"),   row=3, col=1)
        fig.add_trace(go.Scatter(x=dates, y=sig_l,  line=dict(color="#e3b341", width=1.5), name="Signal"), row=3, col=1)
...
st.markdown("## 📡 Signal Screener")
st.markdown(
    f"Screening **{', '.join(exchange_choice)}** · "
...
if run:
    ...
    m4.metric("Oversold RSI (<30)", sum(1 for r in results if r.get("rsi", 50) < 30))
    ...
    for r in results:
        ...
        ma_trend = (
            "🟢 50 > 200" if sma50 and sma200 and sma50 > sma200 else
            "🔴 50 < 200" if sma50 and sma200 else "—"
        )
